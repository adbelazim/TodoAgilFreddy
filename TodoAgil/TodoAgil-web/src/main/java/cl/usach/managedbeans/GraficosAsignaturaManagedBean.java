...
@ViewScoped
public class GraficosAsignaturaManagedBean implements Serializable {
    @EJB
    ...
    public int getIdTableroSeleccionado() {
        return idTableroSeleccionado;
    }

    public void setIdTableroSeleccionado(int idTableroSeleccionado) {
        this.idTableroSeleccionado = idTableroSeleccionado;
    }
    ...
    public void inicialChartCero(){
        lineChartModel = new LineChartModel();
        ChartSeries serie1 = new ChartSeries();
        serie1.set(0,0);
        lineChartModel.addSeries(serie1);
        lineChartModel.setTitle("");
        lineChartModel.setLegendPosition("e");
        lineChartModel.setShowPointLabels(true);
        lineChartModel.getAxes().put(AxisType.X, new CategoryAxis("Días"));
    ...
    public void crearGraficos(){
        if(idTableroSeleccionado != 0){
            Tablero tablero = tableroFacade.buscarPorId(idTableroSeleccionado);
            if(tablero != null){
                lineChartModel = buscarDatosGraficos(tablero);
                lineChartModel.setTitle(tablero.getNombreTablero());
                lineChartModel.setLegendPosition("e");
                lineChartModel.setShowPointLabels(true);
                lineChartModel.getAxes().put(AxisType.X, new CategoryAxis("Días"));
                Axis yAxis = lineChartModel.getAxis(AxisType.Y);
