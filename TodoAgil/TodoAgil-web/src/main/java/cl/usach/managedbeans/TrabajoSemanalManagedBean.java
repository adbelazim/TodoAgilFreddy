...
@ViewScoped
public class TrabajoSemanalManagedBean implements Serializable {
    @EJB
    ...
    public BarChartModel buscarGrafico(){
        ...
        if(sesionManagedBean.checkUserAlumno()){              
            if(sprintGruposFacade.existePorUsuarioSMasterYAsignatura(usuario, asig)){
                List<SprintGrupos> sprintGrupos = sprintGruposFacade.buscarPorUsuarioSMasterYAsignatura(usuario, asig);
                List<Usuario> usuarios = new ArrayList<>();
                for (SprintGrupos sgrupo: sprintGrupos) {
                    List<Usuario> usrsAux = equipoFacade.buscarUsuariosPorSprintGrupo(sgrupo);
                    for (Usuario usrAux : usrsAux) {
                        if(!usuarios.contains(usrAux)){
                            usuarios.add(usrAux);
                        }
                    }
                }
